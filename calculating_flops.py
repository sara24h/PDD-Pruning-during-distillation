...
parser.add_argument("--arch", default=None, type=str, help="arch", required=True)  # اضافه کردن required=True
...
# بررسی اینکه آیا GPU مورد نظر وجود دارد یا خیر
if args.gpu is not None:
    torch.cuda.set_device(args.gpu)

# ایجاد یک متغیر مدل با مقدار اولیه None
model = None
...
if args.arch == 'cvgg11_bn_small':
    ...
    model = cvgg11_bn_small(finding_masks=False, num_classes=args.num_classes, batch_norm=True)
    if args.gpu is not None:
        model = model.cuda()
    model.classifier[1] = torch.nn.Linear(398, 512)
    if args.gpu is not None:
        model.classifier[1] = model.classifier[1].cuda()
...
elif args.arch == 'resnet20_small':  # استفاده از elif به جای if
    ...
    model = resnet20(finding_masks=False, in_cfg=in_cfg, out_cfg=out_cfg, num_classes=args.num_classes)
    if args.gpu is not None:
        model = model.cuda()
    mask = torch.load('/kaggle/working/pretrained_model/resnet56/cifar10/cifar10_T_resnet56_S_resnet20_mask.pt')
    ckpt = torch.load('/kaggle/working/pretrained_model/resnet56/cifar10/cifar10_resnet20.pt', map_location='cuda:%d' % args.gpu if args.gpu is not None else 'cpu')  # 要手动调整
    model.load_state_dict(ckpt)

# بررسی اینکه آیا مدل تعریف شده است یا خیر
if model is None:
